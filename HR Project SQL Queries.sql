...
select sum(employee_count) as Employee_Count from hrdata;
	--if you want to filter by education 
	select sum(employee_count) as employee_count from hrdata
	where education = 'High School'
	--if you want to filter by department 
	select sum(employee_count) as employee_count from hrdata
	where department = 'Sales'
	--if you want to filter by education field
	select sum(employee_count) as employee_count from hrdata
...
select count(attrition) from hrdata where attrition='Yes';
	...
	from hrdata;
		--if you want to filter Attrition Rate by department 
		select 
	...
	--if you want to filter by department and education field
	select count(attrition) from hrdata where attrition='Yes' and department = 'R&D' and education_field = 'Medical'
	--if you want to filter by department, education field, and education 
...
where attrition='Yes';
	--if you want to filter by gender 
	select sum(employee_count) - (select count(attrition) from hrdata  where attrition='Yes') from hrdata
	where gender = 'Male'
...
order by count(attrition) desc;
	...
	group by gender
	order by count(attrition) desc;
	--if you want to filter by education and department 
	select gender, count(attrition) as attrition_count from hrdata
...
--No of Employee by Age Group Power BI 
